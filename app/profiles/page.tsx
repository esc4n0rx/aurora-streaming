...
export default function ProfileSelection() {
  ...
  const [profiles, setProfiles] = useState([
    { id: 1, name: "John", avatar: "/placeholder.svg?height=120&width=120", isKids: false },
    { id: 2, name: "Sarah", avatar: "/placeholder.svg?height=120&width=120", isKids: false },
    { id: 3, name: "Kids", avatar: "/placeholder.svg?height=120&width=120", isKids: true },
  ])
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-black via-[#18181b] to-[#23272f] text-white flex items-center justify-center p-6">
        ...
        <div className="flex flex-wrap justify-center gap-10 mb-12">
          {profiles.map((profile) => (
            ...
            >
              ...
              <h3 className="text-lg font-semibold group-hover:text-blue-400 transition-colors mb-1">{profile.name}</h3>
            </div>
          ))}
      ...
      {/* Modais de adicionar/editar/remover (placeholders) */}
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
          <div className="bg-gray-900 rounded-2xl p-8 shadow-2xl w-full max-w-md">
            <h2 className="text-2xl font-bold mb-4">Adicionar Perfil</h2>
            <p className="text-gray-400 mb-6">Funcionalidade em breve.</p>
            <Button onClick={() => setShowAddModal(false)} className="w-full">Fechar</Button>
          </div>
        </div>
      )}
      {showEditModal && (
      ...
      {showRemoveModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
          <div className="bg-gray-900 rounded-2xl p-8 shadow-2xl w-full max-w-md">
            ...
            <p className="text-gray-400 mb-6">Funcionalidade em breve.</p>
            <Button onClick={() => setShowRemoveModal(null)} className="w-full" variant="destructive">Fechar</Button>
          </div>
        </div>
      )}
    </div>
